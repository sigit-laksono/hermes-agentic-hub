...
import { Check, Copy, FileCode, Terminal, Database, Settings, Workflow } from 'lucide-react'
...
function getLanguageLabel(lang: string = ''): { label: string; icon: React.FC<{ className?: string }> } {
  ...
  switch (norm) {
    ...
    case 'mermaid':
      return { label: 'Mermaid Diagram', icon: Workflow }
    default:
...
export const CodeBlock: React.FC<CodeBlockProps> = ({
  ...
  if (normalizedLang === 'mermaid') {
    return <MermaidViewer code={code} className={className} />
  }
  ...
  return (
    <div className={`my-3 rounded-2xl border border-[#E7E5E4] dark:border-[#2A2524] bg-slate-50 dark:bg-[#14161C] overflow-hidden shadow-xs text-xs font-mono select-text group/code ${className}`}>
      {/* Header toolbar */}
      <div className="flex items-center justify-between px-3.5 py-2 bg-slate-100 dark:bg-[#191C21] border-b border-[#E7E5E4] dark:border-[#2A2524] select-none">
        <div className="flex items-center gap-2">
          <IconComponent className="w-3.5 h-3.5 text-[#F97316]" />
        ...
        <button
          onClick={handleCopy}
          type="button"
          className="flex items-center gap-1.5 px-2 py-1 rounded-md text-[11px] font-sans font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-200/70 dark:bg-slate-800/60 hover:bg-slate-300 dark:hover:bg-slate-700/80 transition-all cursor-pointer"
        ...
        >
          {copied ? (
            ...
            <>
              <Copy className="w-3 h-3 text-slate-500 group-hover/code:text-slate-800 dark:group-hover/code:text-slate-200" />
              <span>Copy Code</span>
            </>
          )}
        </button>
